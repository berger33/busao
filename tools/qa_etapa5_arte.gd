...
func _unlock_up_to_phase_16() -> void:
    var save: Node = _game.get_node("/root/GameSave")
    for i in range(2, 15):
...
func _scenario_x() -> void:
    ...
    # ETAPA 9 — fases 1-15 viraram níveis autorais; o leiaute padrão fica nas
    # fases ainda procedurais (ex.: fase 16, índice 15).
    _unlock_up_to_phase_16()
    _start_phase(15)
    var sem_nivel_e: bool = _deck_cobre(-3.25)
    var sem_nivel_c: bool = _deck_cobre(0.0)
    print("deck fase16: E=%s C=%s (leiaute padrão)" % [str(sem_nivel_e), str(sem_nivel_c)])

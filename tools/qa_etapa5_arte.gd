...
func _scenario_x() -> void:
    ...
    # ETAPA 13 — fases 1-35 viraram níveis autorais; o leiaute padrão fica nas
    # fases ainda procedurais (ex.: fase 36, índice 35).
    _unlock_up_to_phase_36()
    _start_phase(35)
    var sem_nivel_e: bool = _deck_cobre(-3.25)
    var sem_nivel_c: bool = _deck_cobre(0.0)
    print("deck fase36: E=%s C=%s (leiaute padrão)" % [str(sem_nivel_e), str(sem_nivel_c)])

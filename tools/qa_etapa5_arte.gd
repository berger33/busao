...
func _unlock_up_to_phase_11() -> void:
    var save: Node = _game.get_node("/root/GameSave")
    for i in range(2, 10):
...
func _scenario_x() -> void:
    ...
    # ETAPA 8 — fases 1-10 viraram níveis autorais; o leiaute padrão fica nas
    # fases ainda procedurais (ex.: fase 11, índice 10).
    _unlock_up_to_phase_11()
    _start_phase(10)
    var sem_nivel_e: bool = _deck_cobre(-3.25)
    var sem_nivel_c: bool = _deck_cobre(0.0)
    print("deck fase11: E=%s C=%s (leiaute padrão)" % [str(sem_nivel_e), str(sem_nivel_c)])

...
func _scenario_x() -> void:
    ...
    # ETAPA 12 — fases 1-30 viraram níveis autorais; o leiaute padrão fica nas
    # fases ainda procedurais (ex.: fase 31, índice 30).
    _unlock_up_to_phase_31()
    _start_phase(30)
    var sem_nivel_e: bool = _deck_cobre(-3.25)
    var sem_nivel_c: bool = _deck_cobre(0.0)
    print("deck fase31: E=%s C=%s (leiaute padrão)" % [str(sem_nivel_e), str(sem_nivel_c)])

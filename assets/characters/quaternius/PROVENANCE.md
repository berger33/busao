...
- A integração monta o corpo base, divide a malha de pele por regiões quando necessário e anexa meshes de roupa skinned ao mesmo esqueleto. Para a identidade Nina Creator, o código do projeto acrescenta top texturizado, shorts jeans, botas, telefone, tatuagem, brincos, pulseiras e lantejoulas por `BoneAttachment3D`; esses detalhes usam texturas locais e acompanham os ossos. O controlador `scripts/runner_character.gd` seleciona os clips por estado de corrida, salto e agachamento; só cria um fallback simples se a importação do asset principal falhar.

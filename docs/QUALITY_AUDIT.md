...
- **Personagem principal e NPCs:** catálogo de 20 identidades, dez masculinas e dez femininas (lote 1 urbano + lote 2 "Turma do Ponto 2"), com representação baseada no humanoide Quaternius GLTF/skinned real. Corpo, rosto, olhos e cabelo usam o asset Universal Base Characters; roupa modular e mapas PBR vêm de meshes separados. Para Nina Creator, `runner_character.gd` mantém top, shorts jeans, botas, telefone, tatuagem, brincos, pulseiras, lantejoulas e metal em `BoneAttachment3D`, todos acompanhando o esqueleto. `world_character.gd` reutiliza o mesmo humanoide para velha e camelô, em vez de cabeças/cápsulas. `world_animal.gd` concentra o cachorro caramelo 3D com anatomia e animação de cauda/patas, as aves urbanas articuladas (pombo, passarinho, gaivota e urubu) e a fauna de interior (capivara, cavalo, boi, macaco e caranguejo). Todo animal compartilha o mesmo contrato de poses do corredor — `idle`, `run`, `jump`, `crouch` parado e `crouch` em movimento, exposto por `set_motion`/`set_pose`: o caramelo faz a reverência (play bow) antes da perseguição e salta de empolgação durante a corrida; as aves têm voo com batida de asas, planeio com asas coladas, decolagem empinada e agachado bicando o chão, tanto na fauna aérea (`enable_flight_cycle`) quanto na fauna de chão da calçada (`enable_ground_behavior`); capivara se deita, cavalo e boi pastam com o pescoço no chão, macaco espia com a cauda enroscada e caranguejo anda de lado com pinças articuladas. A fauna de interior aparece nos capítulos dos especiais 14, 32, 38 e 39 via chave `"fauna"` do cenário, sempre decorativa e sem alterar o contrato de 13 obstáculos. A escala de 2,15 m, os pés no piso, a sombra de contato e os clips de locomoção são preservados, sem montar o corpo principal com cápsulas.
...
0. Lote 2 de corredores ("Turma do Ponto 2") adicionado ao catálogo: 10 identidades novas com paletas, efeitos ligados em `_start_run` (velocidade, pulo, dash rápido, coração extra, câmera lenta, ímã, escudo e espera do ônibus +2s da Motorista Cida), acessórios `BoneAttachment3D` por perfil e rotação de perfis nos pedestres velha/camelô; a loja ganhou rolagem por arraste na grade de personagens. Preflight e gdparse atualizados para 20 corredores. Lote 1 de poses animais: `world_animal.gd` passou a compartilhar o contrato de poses do corredor (idle/run/jump/crouch) entre o caramelo e as aves urbanas, com fauna de chão na calçada. Lote 2 de poses animais (igualação completa): criadas capivara, cavalo, boi, macaco e caranguejo — citados nos especiais das fases 14, 32, 38 e 39 — já com o contrato completo, incluindo a variante agachado-em-movimento agora visualmente distinta em todas as dez espécies; a fauna de interior aparece por capítulo via chave `"fauna"` de `scenario_data.gd`.
...
## Lote 5 — pele, pelo e penas PBR (2026-09-18)

- `tools/generate_textures.py` ganhou `pele_realista` (poros, vermelhidão sutil, oleosidade variável), `pelo_realista` (fios direcionais com smear diagonal e sombra entre mechas) e `pena_realista` (barbas diagonais, pente fino senoidal e iridescência sutil); o jeans ganhou normal+roughness da sarja (twill em relevo). Bases neutras claras para preservar as paletas tingidas por `albedo_color`.
- `game_3d.gd`: superfície `skin` migrou de `pele_suave.svg` para os novos mapas (normal 0.2 — poros sutis); const antiga removida com verificação de zero referências.
- `world_animal.gd`: as 10 espécies usam pelo/pena/borracha/metal PBR — o `_material` agora aplica normal+roughness por textura (pelo 0.7 mantendo subsurface scattering, pena 0.6, borracha 0.3, metal 0.4) com `uv1_scale` calibrado; coleiras e placas em metal riscado; bicos e patas em borracha granulada.

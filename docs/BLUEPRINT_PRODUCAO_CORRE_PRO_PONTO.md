...
> Nota: a imagem `media/referencia-blueprint-runner.jpg` precisa ser adicionada ao repositório (o upload não persistiu no sandbox). Até lá, a referência textual da seção 3 é a fonte.

Catálogo complementar: [plano das 50 fases](PLANO_50_FASES_CORRE_PRO_PONTO.md) *(a criar)*.
...
## Anexo A — Reconciliação com o estado do branch `arena/01a0bfcf-busao` (20/09/2026)

Registrado por quem está implementando, para que o blueprint e o código não divirjam em silêncio.

| Item do blueprint | Situação real no branch | Ação |
| :--- | :--- | :--- |
| `validate_project.py` falhava (manifesto SHA-256) | **Corrigido** — manifesto regenerado; `PRE-FLIGHT OK`. | — |
| `check_gdscript.py` sem `lark` | Instalado via `tools/blender/make_env.sh` (venv fora do repo), mas o script falha por incompatibilidade de versão do `gdtoolkit` (parser). `gdparse`/`gdlint` funcionam. | M0: fixar versão do gdtoolkit ou trocar o checker. |
| Protagonista "conferir o GLB efetivamente carregado" | **Bug crítico encontrado e corrigido:** todos os GLBs eram Draco → Godot importava sem geometria (só a sombra). Regenerados sem Draco. `julia.glb` já é malha única (`build_corredora_fase1.py`). | Validar no aparelho (pendente do usuário). |
| Tráfego "carros vindo por trás como principal desafio" | Rodas com pivô próprio; veículos 1,2–1,85× a velocidade do jogador; car-following sem atravessamento. Mantém a rua como faixa jogável **por enquanto**. | Decisão do blueprint (3 corredores na calçada) fica para M1; não mexer em `LANE_X` isoladamente. |
| Prazo do ônibus contado na corrida | **Não implementado.** `_update_run` ainda faz `at_stop` → contagem → vitória. | **M1 — próxima tarefa de código.** |
| 20 personagens em malha única | Plano em `PLANO_PRODUCAO_PERSONAGENS_BLENDER.md`. | **Rebaixado**: só a protagonista até M4 aprovado (blueprint §2). Os outros 19 ficam com os GLBs atuais (visíveis, sem Draco). |
| `PLANO_50_FASES_CORRE_PRO_PONTO.md` | Não existe ainda. | Criar em M5+. |
| Imagem de referência | Upload não persistiu no sandbox. | Usuário adiciona `docs/media/referencia-blueprint-runner.jpg`. |

**Ordem de execução adotada a partir daqui:** M0 (fechar) → M1 (prazo real + regras de dano por obstáculo + 3 corredores na calçada) → M2 (câmera + Júlia validadas no trecho de 28 m) → M3 → M4 piloto.

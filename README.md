...
- **Materiais PBR consistentes**: asfalto com brita e rachaduras, calçada portuguesa no padrão Copacabana, fachadas de reboco e tijolo com janelas/peitoril/AO em textura, céus equiretangulares 2K com nuvens e sol, tonemap ACES, pintura automotiva com flakes metálicos, madeira com veios e nós, metal pintado com riscos e lascas, concreto com juntas de fôrma, terra vermelha com pedrinhas, tecido tramado e folhagem com profundidade — tudo gerado por `tools/generate_textures.py` (albedo + normal + roughness, tileable e determinístico). Veículos procedurais podem ser substituídos por modelos GLB CC0 soltos em `assets/vehicles/` (ver `docs/PLAYSTORE_AUDIT.md`).
...
scripts/world_animal.gd      Animais 3D (caramelo e aves) com o contrato de poses do corredor
...
assets/characters/humanos_originais/  Humano M/F 100% original Blender, rigged/skinned 26 bones + 6 clips, sem CC0 (Lote 26 remove quaternius)
...
O catálogo continua oferecendo vinte identidades e progressão de loja, mas a representação de gameplay agora usa **humano 100% original Blender** (`Humano_M/F.glb` em `assets/characters/humanos_originais/`): corpo, rosto, olhos e cabelo modelados do zero, 26 bones + dedos, 6 clips (`Idle_Loop`, `Walk_Loop`, `Sprint_Loop`, `Jump_Loop`, `Crouch_Idle/Fwd`) e materiais `QuaterniusSkin/Hair/Camisa/Calca/Sapato` para tintagem dinâmica. Lote 26 removeu totalmente o legado Quaternius/CC0 (86 MB). A identidade Nina Creator recebe camada de figurino texturizada de top, shorts jeans e botas, além de acessórios metálicos e celular ancorados nos ossos via `BoneAttachment3D`. O lote 2 segue o mesmo padrão com boné/sacola, chapéu de vaqueiro, colar, touca/bandeja, gravata/livro, bandana/avental, lenço/bolsa, gorro/crachá, faixa e quepe. Os NPCs de obstáculo reutilizam o mesmo corpo profissional no modo mundo, e o caramelo possui animal 3D próprio original com cauda/patas. O modelo é girado 180° (`MODEL_FACING_YAW`) porque o GLTF é exportado olhando para `+Z` enquanto o corredor avança para `-Z`. A cadência do `AnimationPlayer` acompanha a velocidade real do cenário (`Sprint_Loop` cobre ~4 m/s por ciclo), evitando pé patinar. Manifesto SHA-256 em `assets/characters/humanos_originais/PROVENANCE.md`. As cores/estilos da loja permanecem dados de progressão.

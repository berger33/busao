...
func _build_lamp(pos: Vector3, accent: Color) -> void:
    # drop-in: poste.glb (haste, braco curvo e luminaria com lente emissiva)
...
func _create_bus_stop(total: float) -> void:
    ...
    # drop-in: ponto.glb (abrigo com colunas, teto com testa amarela, vidros
    # e banco) substitui o abrigo de caixas; a placa fica procedural (skin)
...
func _optional_prop(file: String) -> Node3D:
    # Drop-in de mobiliario: se existir assets/props/<file>, ele substitui a
    # peca procedural (ver assets/props/LEIA-ME.md — escala real, frente +Z).
    var chave := "props/" + file
    if not _glb_cache.has(chave):
        var packed_prop: PackedScene = null
        var caminho := "res://assets/props/" + file
        if ResourceLoader.exists(caminho):
            packed_prop = load(caminho) as PackedScene
        _glb_cache[chave] = packed_prop
    var prop_cache: PackedScene = _glb_cache[chave]
    if prop_cache == null:
        return null
    var prop_node := prop_cache.instantiate() as Node3D
    if prop_node == null:
...
func _build_sidewalk_obstacle(parent: Node3D, kind: String) -> void:
    ...
    match kind:
        ...
        "hydrant":
            # drop-in: hidrante.glb substitui o hidrante procedural
        ...
        "payphone":
            # drop-in: orelhao.glb (capuz com abertura, teclado e moedeiro)
        ...
        "cone":
            # drop-in: cone.glb com faixas refletivas
        ...
        "vendor":
            # drop-in: carrinho.glb (tampo, vitrine, toldo listrado) substitui
            # as caixas do carrinho; o camelô continua procedural (personagem)
        ...
        "bench":
            # drop-in: banco.glb (ripas de madeira, pes de aco)
